...
<body>
    ...
    <!-- основная часть -->
    <h2 class="gallery__header">Кабинет</h2>
    <div class="container">
        <h2 style="margin-bottom: 20px;">История заказов</h2>
        <table id="userTable" class="cell-border hover row-border" style="width:100%; overflow-x: auto;">
            <thead>
                <tr>
                    <th>№ заказа</th>
                    <th>Стоимость</th>
                    ...
                    foreach ($cep as $num) {
                        ...
                        echo '
                                        <tr>
                                            <td>' . $num[0] . '</td>
                                            <td>' . $num[2] . '</td>                      
                                            <td><a class="button_table" href="/pages/order_detail.php?num=' . $num[0] . '">Открыть</a></td>
        ...
        <h2 style="margin-bottom: 20px;">Список желаемого</h2>
        <table style="width:100%; overflow-x: auto;">
            ...
            <tbody>
                <?php
                require_once '../vendor/set.php';
                $sql = "SELECT product_id ,COUNT(product_id) FROM likes WHERE `user_id` = " . $_SESSION["user"]["id"] . " GROUP BY product_id";
                $cep  = $mysqli->query($sql);
                if (mysqli_num_rows($cep) > 0) {
                    $numb = 0; 
                    $cep = mysqli_fetch_all($cep);
                    foreach ($cep as $num) {
                        ...
                        echo '
                                        <tr>
                                            ...
                                            <td><a class="button_table" href="/vendor/delete_likes.php?num=' . $num[0] . '">Удалить</a></td>
                                        </tr>                              
                                            ';
                    }
                } else {
                ...
                ?>
                <tr>
                    <td style="padding: 10px 20px; background: #56433D; color: #F9C941; border-right: 2px solid; font-size: 0.9em;">Стоимость</td>
                    <td class="totaler"></td>
                </tr>
            </tbody>
        </table>
        ...
        <h2 style="margin-bottom: 20px;">Изменить логин или пароль</h2>
            ...
            <div class="about__wrapper" style="display: block; width:100%; text-align:center;">
                <div class="about__form">
                    <form action="/vendor/login.php" method="post" class="about__form-action">
                        ...
                        <input type="text" name="login" require style="width:40%;" value="<?= $_SESSION["user"]["login"] ?>"><br>
                        <label for="">Пароль</label><br>
                        <input type="password" name="password" require style="width:40%;"><br>
                        <button style="display: inline; width:30%;" type="submit" class="about__form-button">Изменить</button><br><br>
                    </form>
                </div>
            </div>
        </div>

        <a class="button-4" href="/vendor/logout.php">Выход</a>
